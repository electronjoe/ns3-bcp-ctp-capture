...
- `sweeps/random_tinfo_avg/ring6_random_tinfo_avg.csv` captures five randomized-fault trials per controller over `Tinfo ∈ {0, 0.5, 1, 2, 4, 8, 12, 16, 24, 32}` (blocking arc 4→5) with `count=5000`, `rate=10`, `simTime=600` so each row reflects ≈600 simulated seconds.
- `sweeps/random_multiarc_avg/ring6_random_multiarc_avg.csv` merges four matching 600 s sweeps (`badArc=1,2`, `0,1`, `3,4`, `5,0`) each covering `Tinfo ∈ {0, 2, 4, 8, 16}` with five trials. Individual per-arc logs/CSVs live under `sweeps/random_multiarc_avg/arc*/`, and plots with ±σ shading were generated via the enhanced `plot_sweep.py`.
